...
const History: NextPage = () => {
  ...
  const { data: questBonus } = api.quests.getQuestBonusByDriverId.useQuery({
    driverId: DUMMY_DRIVER_ID,
  });
            ...
            <Disclosure>
              <Disclosure.Button>
                <div className="flex justify-between">
                  ...
                  <div className="underline">
                    {addCurrency(formatNumbersWithCommas(questBonus ? questBonus.reduce((prev, curr) => { return prev + curr.bonus}, 0): 0), "SG")}
                  </div>
                </div>
              </Disclosure.Button>
              <Disclosure.Panel>
                <div className="mb-2 flex flex-col gap-2 text-sm text-gray-600 opacity-70">
                  {questBonus?.map((quest) => {
                    return (
                      <div key={quest.title} className="flex justify-between">
                        <div>{quest.title}</div>
                        <div>{addCurrency(formatNumbersWithCommas(quest.bonus), "SG")}</div>
                      </div>
                    );
                  })}
                </div>
